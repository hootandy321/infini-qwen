...
void save_tensor_debug(const std::shared_ptr<Tensor> &tensor, const std::string &name, 
    ...
    if (has_issues) {
        std::ostringstream oss;
        oss << "⚠ Tensor " << name << " has data quality issues - check " << filename << "\n";
        write_debug_log(oss.str());
    }
}
...
void save_tensor_debug_f16(const std::shared_ptr<Tensor> &tensor, const std::string &name, 
    ...
    std::ostringstream oss;
    oss << "  Saved FP16 debug tensor: " << filename << "\n";
    write_debug_log(oss.str());
}
...
void createQwen3DeviceResource(DeviceQwen3Resource *rsrc, const Qwen3Meta *meta,
    ...
    // 此池在推理期间管理临时张量以避免频繁的 malloc/free
        // 增加内存池大小
    auto memory_pool = std::make_shared<MemoryPool>(512 * 1024 * 2048); // 从128MB增加到512MB
    
    if (g_debug_enabled && idev == 0) {
        write_debug_log("Memory pool size increased to 512MB\n");
    }
...
void inferQwen3DeviceBatch(const Qwen3Meta &meta, DeviceQwen3Resource &rsrc,
    ...
    if (dt_logits != dt_logits_corrected) {
        if (g_debug_enabled && idev == 0) {
            std::ostringstream oss;
            oss << "WARNING: Corrected dt_logits from " << dt_logits 
                << " to " << dt_logits_corrected << " to match weights\n";
            write_debug_log(oss.str());
        }
    ...
    if (g_debug_enabled && idev == 0) {
        ...
        if (g_debug_enabled && idev == 0 && i == 0) {
            std::ostringstream oss;
            oss << "DEBUG: Copying embedding for token " << tokens[i] << " to position " << i << "\n"
                << "  Source: " << rsrc.w_in_embd->data(tokens[i] * d) << " (offset: " << (tokens[i] * d) << ")\n"
